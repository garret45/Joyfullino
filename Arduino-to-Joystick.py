...
class Application(Frame, object):
    def __init__(self, master=None):
        ...
        drop = OptionMenu(root, self.selected_port, *port_list).grid(row=0, column=1)
        ...
        start_button = Button(root, text=" Start Joystick ", command=self.startJoy).grid(row=3, column=1)
        stop_button = Button(root, text=" Stop Joystick ", command=self.stopJoy).grid(row=4, column=1)
